...
_setup_normal_io(slurmd_job_t *job)
{
	...
	if (!job->batch) {
		...
		debug3("The io port is:%d\n", srun->ioaddr.sin_port);
...
job_manager(slurmd_job_t *job)
{
	...
	{
		debug3("OK, io initialization failed!\n");
		goto fail2;
	}*/
...
_fork_all_tasks(slurmd_job_t *job, bool *io_initialized)
{
	...
	{
		debug3("OK, I am doing user managed io!\n");
		rc = _setup_user_managed_io(job);
	}
	else
	{
		debug3("OK, I am doing normal IO\n");
		rc = _setup_normal_io(job);
	}
	debug3("The normal IO created is:%d\n", rc);
	/*
	...
	debug3("Now, the normal IO created of log init is:%d\n", rc);
	if (rc) {
	...
	for (i = 0; i < job->node_tasks; i++) {
		...
		if (i == (job->node_tasks - 1)) {
			...
			debug3("start polling on the last task!\n");
...
//	jobacct_gather_set_proctrack_container_id(job->cont_id);
	...
	for (i = 0; i < job->node_tasks; i++) {
		...
		if (pdebug_trace_process(job, job->task[i]->pid)
		    == SLURM_ERROR)
		{
			debug3("OK, prepare process for attach by parallel debugger false!\n");
			rc = SLURM_ERROR;
		}
	}
